package com.otabi.firestar.smwtheme;
...
import com.otabi.firestar.smwtheme.BuildConfig;
import com.otabi.firestar.smwtheme.R;
...
public class SubstratumLauncher extends Activity {
    ...
    private void startAntiPiracyCheck() {
        if (ThemerConstants.PIRACY_CHECK && ThemerConstants.BASE_64_LICENSE_KEY.length() == 0)
            Log.e("SubstratumAntiPiracyLog", PiracyCheckerUtils.getAPKSignature(this));

        PiracyChecker piracyChecker = new PiracyChecker(this);
        if (ThemerConstants.ENFORCE_GOOGLE_PLAY_INSTALL) piracyChecker.enableInstallerId(InstallerID.GOOGLE_PLAY);
        if (ThemerConstants.ENFORCE_AMAZON_APP_STORE_INSTALL)
        ...
        if (ThemerConstants.BASE_64_LICENSE_KEY.length() > 0) {
            piracyChecker.enableGooglePlayLicensing(ThemerConstants.BASE_64_LICENSE_KEY);
        }
        if (ThemerConstants.APK_SIGNATURE_PRODUCTION.length() > 0) {
            piracyChecker.enableSigningCertificate(ThemerConstants.APK_SIGNATURE_PRODUCTION);
        }
        piracyChecker.start();
    }
    ...
    private void showOutdatedSubstratumToast() {
        String parse = String.format(
                ...
                getString(R.string.ThemeName),
                String.valueOf(ThemerConstants.MINIMUM_SUBSTRATUM_VERSION));
        Toast.makeText(this, parse, Toast.LENGTH_SHORT).show();
    }

    private void launchSubstratum() {
        if (ThemerConstants.ENABLE_BLACKLISTED_APPLICATIONS) {
            for (String blacklistedApplication : ThemerConstants.BLACKLISTED_APPLICATIONS)
        ...
        if (ThemerConstants.ENFORCE_MINIMUM_SUBSTRATUM_VERSION) {
            try {
                PackageInfo packageInfo = getApplicationContext()
                        .getPackageManager().getPackageInfo(SUBSTRATUM_PACKAGE_NAME, 0);
                if (packageInfo.versionCode >= ThemerConstants.MINIMUM_SUBSTRATUM_VERSION) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (ThemerConstants.ENFORCE_INTERNET_CHECK) {
            if (sharedPref.getInt("last_version", 0) == BuildConfig.VERSION_CODE) {
                if (ThemerConstants.THEME_READY_GOOGLE_APPS) {
                    detectThemeReady();
                } else {
                    launch();
                }
    ...
    private void launch() {
        if (ThemerConstants.PIRACY_CHECK && !BuildConfig.DEBUG) {
